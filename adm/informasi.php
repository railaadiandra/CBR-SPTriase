<?php
	...
	$jml_data = mysqli_num_rows(mysqli_query($conn,"select * from informasi"));
	...
	if(isset($_GET['hal'])){
			...
			$s = mysqli_query($conn,"select * from informasi order by id_informasi desc limit ".(($hal - 1) * $jml_per_halaman).", $jml_per_halaman");
		
	}else{
		$s = mysqli_query($conn,"select * from informasi order by id_informasi desc limit 0, $jml_per_halaman")
				or die ($conn);
	}
?>
